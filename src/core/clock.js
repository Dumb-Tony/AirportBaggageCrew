/* GameClock — the single owner of simulation time. GDD §21.3, §5.
 *
 * THE SACRED SCHEDULE RUNS ON THIS AND NOTHING ELSE. No system may read Date.now(),
 * performance.now(), setTimeout or setInterval to decide when something happens. A
 * flight advances because simTimeMs crossed its threshold, never because a browser
 * timer fired. That is what makes a shift replayable and testable (GDD §31.1.6).
 *
 * Fixed step with an accumulator: real frame time is clamped, scaled, banked, and
 * spent in whole 1/60s steps. The clock advances simTimeMs ITSELF, inside advance(),
 * as it calls the step callback — so simulation time and steps executed cannot drift
 * apart no matter what the caller does.
 */

export class GameClock {
  /**
   * @param {number} stepMs      fixed simulation step (GDD recommends 1/60 s)
   * @param {number} maxFrameMs  frame gaps longer than this are discarded, not banked.
   *   Backgrounded tabs hand back multi-second deltas; without this the sim would try
   *   to catch up over hundreds of steps and the airport would lurch forward. GDD
   *   §28.2 requires "no enormous unbounded simulation catch-up".
   */
  ...
  /** Debug: run the simulation forward without waiting for real frames. GDD §21.8. */
  skipMs(ms, onStep) {
  ...
  /** mm:ss for the HUD. Formatting lives here so no two panels disagree. */
  static formatMs(ms) {
    if (ms < 0) ms = 0;
